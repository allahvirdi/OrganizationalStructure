...
**Session مربوطه:** `Session-20260925-AuthMeAnonymousProbe`

## رفع خطای ۴۰۱ روی `/api/v1/auth/me` (بررسی نشست) — 2026-09-25 — DEC-036
- گزارش کارفرما: کنسول مرورگر روی صفحهٔ ورود، `GET /api/v1/auth/me 401` نشان می‌داد.
- ریشه: اکشن `me` با `[Authorize]` محافظت شده بود و BFF برای نبودِ نشست، `AuthenticateResult.Fail("نشست معتبر نیست.")` می‌داد ⇒ ۴۰۱. این «وضعیت عادی بازدیدکنندهٔ واردنشده» بود، نه خطای واقعی؛ ولی در کنسول و ذهن کاربر به‌عنوان خطا دیده می‌شد.
- اصلاح بک‌اند: `[AllowAnonymous]` + بازگشت `204 No Content` وقتی `User` احراز نشده باشد/`userId` نداشته باشد + پاک‌کردن کوکی نشست کهنه. سایر Endpointها همان ۴۰۱ fail-closed را دارند.
- اصلاح فرانت‌اند: `fetchMe` تایپ `CurrentUser | null` شد (۲۰۴ ⇒ `null`؛ تمایز «ناشناس» از «خطا»).
- تأیید زنده روی بک‌اند اجراشده: کاربر ناشناس ⇒ `/api/v1/auth/me` = **۲۰۴** و `/api/v1/posts` = **۴۰۱**.
- اعتبارسنجی: بیلد بک‌اند ۰ هشدار/۰ خطا · **۲۴۷ تست سبز** (Domain 37 + Application 98 + Infrastructure 27 + Architecture 4 + Integration 81) · `tsc` ✅ · `eslint` ✅ · `next build` ✅.
- نکته عملیاتی: تشخیص «خطای واقعی احراز هویت» از «ناشناس بودن» — ۴۰۱ فقط برای Endpointهای محافظت‌شده و ورود ناموفق باقی می‌ماند.
...
- دیالوگ انتساب مسئولیت به پست بازطراحی شد: شناسه خام پست حذف و با دو `Autocomplete` جایگزین شد — انتخاب سازمان (دسترسی درختی) + جستجوی پست در سازمان انتخاب‌شده (کامیت `d3232ad`).
...
## ادامه — «ثبت حق امضا» به‌جای «ثبت اختیار» + کد خودکار GUID (2026-09-23) — DEC-035
- ماژول Authorities در UI به **«حق امضا»** تغییر نام یافت: منوی اصلی و کارت داشبورد («حق امضاها»)، عنوان/دکمه/جستجوی فهرست، صفحه ایجاد («حق امضای جدید» / «ثبت حق امضا») و برچسب‌های صفحه پست‌ها.
- فیلد `Code` از `CreateAuthorityCommand`، اعتبارسنج، هندلر، کنترلر، اسکیمای Zod، API فرانت‌اند و فرم ایجاد حذف شد؛ هندلر `id = Guid.NewGuid(); code = id.ToString();` را تولید می‌کند و بررسی کد تکراری حذف شد (یکتایی ذاتی GUID).
- ریدایرکت پس از ایجاد با شناسه برگشتی (`newId`) انجام می‌شود.
- دیالوگ انتساب حق امضا به پست دقیقاً مانند مسئولیت شد: دراپ‌داون جستجوپذیر سازمان (دسترسی درختی) + جستجوی پست در سازمان انتخاب‌شده؛ فیلد «شناسه پست» حذف شد.
