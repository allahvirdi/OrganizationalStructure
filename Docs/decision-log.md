...
| DEC-006 | 2026-09-14 | افزودن `Region=3` به `OrganizationType` در IAM (با هماهنگی مالک IAM) | Accepted | ADR-009، ADR-014 | کارفرما |
...
| DEC-036 | 2026-09-25 | `GET /api/v1/auth/me` به‌عنوان **بررسی نشست** تعریف شد: برای کاربر ناشناس `204 No Content` برمی‌گرداند (نه ۴۰۱) و کوکی نشست نامعتبر/کهنه را پاک می‌کند؛ سایر Endpointها رفتار fail-closed (۴۰۱) را حفظ می‌کنند. علت: حذف خطای گمراه‌کنندهٔ ۴۰۱ در کنسول مرورگرِ بازدیدکنندهٔ واردنشده. این تصمیم DEC-033 را نقض نمی‌کند (DEC-033 دربارهٔ قرارداد خطای **ورود** ناموفق است). | Accepted | — | کارفرما |
| DEC-037 | 2026-09-25 | Q-011: افزودن `School = 4` به `OrganizationType` در IAM (سطح چهارم کسب‌وکاری: مدرسه، زیرمجموعه منطقه)؛ پیاده‌سازی در مخزن IAM به‌صورت تسک مستقل با هماهنگی مالک IAM، هم‌الگو با DEC-019/ADR-009. مقادیر ۰ تا ۳ بدون تغییر. | Accepted | ADR-014 | کارفرما |
| DEC-038 | 2026-09-25 | Q-012: سامانه ارجاعات سلسله‌مراتبی (گردش کار موارد/نامه‌ها) یک **سامانه مستقل** در کنار `OrganizationalStructure` است؛ ماژول داخلی این مخزن نیست. `OrganizationalStructure` صرفاً ارائه‌دهنده داده پایه ساختار، پست‌ها، حق مسئولیت و حق امضا (از جمله `FindResponsible`) است و سامانه ارجاعات مالک `Item`/`Referral`/`ReferralHistory` و قوانین گردش کار خود می‌باشد. | Accepted | ADR-015 | کارفرما |
...
|---------|-------|-----------|
| —       | (در حال حاضر موردی در انتظار نیست — همه سؤالات از جمله Q-011 و Q-012 بسته شده‌اند) | — |
